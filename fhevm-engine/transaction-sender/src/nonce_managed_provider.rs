...
// Provider fillers without nonce management.
pub type DefaultProviderFillers = JoinFill<
    GasFiller,
...
/// is done by the `NonceManagedProvider` itself. Users can use the default `DefaultProviderFillers` to create a provider.
#[derive(Clone)]
...
impl<P: alloy::providers::Provider<Ethereum> + Clone + 'static> NonceManagedProvider<P> {
    ...
    pub async fn send_transaction(
        ...
        let mut tx = tx.into();
        if let Some(a) = self.signer_address {
            let nonce_manager = self.nonce_manager.lock().await;
            let nonce = nonce_manager.get_next_nonce(&self.provider, a).await?;
            tx.nonce = Some(nonce);
        }
