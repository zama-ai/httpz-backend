...
export function generateSolidityTFHELibAndOverloads(operators: Operator[], fheTypes: FheType[]): string {
  const res: string[] = [];
...
function handleSolidityTFHEEncryptedOperatorForTwoEncryptedTypes(
  ...
  if (lhsFheType.type.startsWith('Euint') && rhsFheType.type.startsWith('Euint')) {
    ...
    res.push(`
    /**
    * @dev Evaluates ${operator.name}(${lhsFheType.type.toLowerCase()} a, ${rhsFheType.type.toLowerCase()} b)  and returns the result.
    */
...
`);
  } else if (lhsFheType.type === 'Ebool' && rhsFheType.type === 'Ebool') {
    res.push(`
    /**
...
`);
  } else if (lhsFheType.type == rhsFheType.type && rhsFheType.type.startsWith('Ebytes')) {
    ...
    res.push(`
      ...
      * @dev Evaluates ${operator.name}(${lhsFheType.type.toLowerCase()} a, ${rhsFheType.type.toLowerCase()} b) and returns the result.
      */
      function ${operator.name}(${lhsFheType.type.toLowerCase()} a, ${rhsFheType.type.toLowerCase()} b) internal returns (ebool) {
          if (!isInitialized(a)) {
          ...
          return ebool.wrap(Impl.${operator.name}(${lhsFheType.type.toLowerCase()}.unwrap(a), ${rhsFheType.type.toLowerCase()}.unwrap(b), false));
      }
  `);
  } else if (lhsFheType.type.startsWith('Eaddress') && rhsFheType.type.startsWith('Eaddress')) {
    res.push(`
      /**
...
function generateSolidityTFHEScalarOperator(fheType: FheType, operator: Operator): string {
  ...
  res.push(`
    /**
    * @dev Evaluates ${operator.name}(${fheType.type.toLowerCase()} a, ${fheType.clearMatchingType.toLowerCase()} b) and returns the result.
    */
...
`);
  ...
  if (!operator.leftScalarDisable) {
    ...
    /**
    * @dev Evaluates ${operator.name}(${fheType.clearMatchingType.toLowerCase()} a, ${fheType.type.toLowerCase()} b) and returns the result.
    */
...
`);
    ...
    res.push(`
    /** 
     * @dev Evaluates ${operator.name}(${fheType.type.toLowerCase()} a, ${getUint(rhsBits)}) and returns the result.
     */
...
function handleSolidityTFHEUnaryOperators(fheType: FheType, operators: Operator[]): string {
  ...
  operators.forEach((op) => {
    if (op.arguments == OperatorArguments.Unary && fheType.supportedOperators.includes(op.name)) {
      res.push(`
          /**
           * @dev Evaluates ${op.name}(${fheType.type.toLowerCase()} value) and returns the result.
           */
