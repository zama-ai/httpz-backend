...
export const generateOverloads = (fheTypes: FheType[]) => {
  ...
  Object.keys(SUPPORTED_FUNCTIONS).forEach((functionName: string) => {
    ...
    fheTypes.forEach((lhsFheType: FheType) => {
      if (lhsFheType.type.startsWith('Euint') && lhsFheType.supportedOperators.includes(functionName)) {
        if (test.unary) {
          let lhsNumber = generateRandomNumber(lhsFheType.bitLength);
          const encryptedTestName = [functionName, lhsFheType.type.toLowerCase()].join('_');
          ...
          fheTypes.forEach((rhsFheType: FheType) => {
            if (rhsFheType.type.startsWith('Euint') && rhsFheType.supportedOperators.includes(functionName)) {
              ...
              if ((test.limit !== 'bits' || only8bits) && !test.scalarOnly) {
                const encryptedTestName = [
                  functionName,
                  lhsFheType.type.toLowerCase(),
                  rhsFheType.type.toLowerCase(),
                ].join('_');
                const encryptedTests: Test[] = [];
                if (!test.lhsHigher) {
                  encryptedTests.push(
